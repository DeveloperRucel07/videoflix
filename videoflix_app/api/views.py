...
class VideoHlsStreamManifestView(APIView):
    permission_classes = [IsAuthenticated]
    authentication_classes = [CookieJWTAuthentication]
    BASE_HLS_DIR = settings.MEDIA_ROOT / 'hls'
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not request.path.endswith("index.m3u8"):
            raise Http404("Invalid HLS manifest endpoint")
        
        candidate = (self.BASE_HLS_DIR / str(movie_id) / resolution / 'index.m3u8').resolve()
        print("HLS manifest path:", candidate)
        if not str(candidate).startswith(str(self.BASE_HLS_DIR.resolve())):
            raise Http404('Invalid HLS manifest path')

        if not candidate.is_file():
            raise Http404('HLS manifest not found')
        
        response = FileResponse(open(candidate, 'rb'), content_type=HLS_CONTENT_TYPE)
        return response


class VideoHlsSegmentView(APIView):
    """
    Serve HLS video segments from MEDIA_ROOT/hls/<movie_id>/<resolution>/<segment>.ts
    """
    permission_classes = [IsAuthenticated]
    authentication_classes = [CookieJWTAuthentication]
    ...
    def get(self, request, movie_id=None, resolution=None, segment=None, *args, **kwargs):
        ...
        candidate = (self.BASE_HLS_DIR / str(movie_id) / resolution / segment).resolve()
        print("HLS manifest path:", candidate)
        if not str(candidate).startswith(str(self.BASE_HLS_DIR.resolve())):
            raise Http404("Invalid segment path")
        if not candidate.is_file():
            raise Http404("Segment not found")
        response = FileResponse(open(candidate, "rb"), content_type=TS_CONTENT_TYPE)
        return response
